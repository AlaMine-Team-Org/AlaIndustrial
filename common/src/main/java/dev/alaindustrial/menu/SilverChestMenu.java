...
/**
 * Menu for the Silver Chest — 45 storage slots (5 rows of 9, one row more than the iron chest's 36)
 * plus the player inventory + hotbar. Backed by the block entity's {@link Container}; does NOT extend
 * {@link MachineMenu} because the silver chest has no energy/progress data to sync — identical
 * structure to {@link IronChestMenu}.
 *
 * <p>Slot coordinates mirror the {@code silver_chest.png} GUI atlas (176×202, one row taller than the
 * iron chest's 176×184): chest grid starts at {@code (8, 18)} with an 18px stride, the player
 * inventory starts at {@code (8, 121)} (iron chest: 103, +18 for the extra chest row), and the hotbar
 * at {@code (8, 179)} (iron chest: 161, +18). The visible GUI panel is 176×202, so
 * {@code imageHeight} on the screen must match.
 */
public class SilverChestMenu extends AbstractContainerMenu {
	/** Top-of-grid Y for the 5 chest rows. */
	private static final int CHEST_TOP_Y = 18;
	/** Top-of-grid Y for the 3 player-inventory rows (iron chest: 103 + 18 for the extra chest row). */
	private static final int PLAYER_INV_TOP_Y = 121;
	/** Top-of-grid Y for the hotbar row (iron chest: 161 + 18). */
	private static final int HOTBAR_TOP_Y = 179;
	...
	private SilverChestMenu(MenuType<?> type, int syncId, Inventory playerInventory, Container chest,
		...
		// Notify the container a player opened it — this drives the opener counter on the silver chest
	...
	public boolean stillValid(Player player) {
		return AbstractContainerMenu.stillValid(access, player, ModContent.SILVER_CHEST.get());
	}
	...
	public void removed(Player player) {
		// Notify the container the player closed it — decrements the opener counter on the silver
