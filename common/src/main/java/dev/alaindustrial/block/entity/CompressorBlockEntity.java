...
public class CompressorBlockEntity extends MachineBlockEntity implements MenuProvider {
	...
	private final RecipeManager.CachedCheck<SingleRecipeInput, AlaProcessingRecipe> recipeCheck =
			ModRecipes.COMPRESSING.newCheck();

	public CompressorBlockEntity(BlockPos pos, BlockState state) {
		super(ModContent.COMPRESSOR_BE.get(), pos, state, EnergyTier.LV, 2,
				Config.machineBuffer, EnergyTier.LV.maxVoltage(), 0L);
		this.maxProgress = Config.scaledDuration(Config.compressorDuration);
	}
	...
	protected int onServerTick(Level level, BlockPos pos, BlockState state) {
		...
		int baseDuration = recipe != null
				? Math.max(1, recipe.energy() / Config.machineEuPerTick) : Config.compressorDuration;
