...
/**
 * Block-entity renderer for the silver chest — a 3D chest model with an animated lid, textured with
 * the mod's own {@code textures/entity/chest/silver.png} (a 64×64 atlas matching the vanilla chest
 * layout, recoloured to silver). Structurally identical to
 * {@link IronChestBlockEntityRenderer}; only the model layer + sprite id differ.
 *
 * <p>Texture source: the silver texture is submitted through the {@code chest} atlas
 * ({@link Sheets#CHEST_MAPPER}), sprite id {@code alaindustrial:silver}, picked up by the vanilla
 ...
 */
public class SilverChestBlockEntityRenderer implements BlockEntityRenderer<SilverChestBlockEntity, ChestRenderState> {
	...
	/** Sprite id for the silver chest texture inside the chest atlas (entity/chest/silver). */
	private static final SpriteId SPRITE = Sheets.CHEST_MAPPER.apply(Industrialization.id("silver"));

	private final ChestModel model;
	private final SpriteGetter sprites;

	public SilverChestBlockEntityRenderer(BlockEntityRendererProvider.Context context) {
		ModelPart root = context.bakeLayer(SILVER_CHEST_LAYER);
	...
	@Override
	public void extractRenderState(SilverChestBlockEntity entity, ChestRenderState state, float partialTicks,
		...
		Direction facing = entity.hasLevel()
				? entity.getBlockState().getValue(SilverChestBlock.FACING)
				: Direction.NORTH;
		state.facing = facing;
		// The silver chest is never part of a double chest — always SINGLE.
