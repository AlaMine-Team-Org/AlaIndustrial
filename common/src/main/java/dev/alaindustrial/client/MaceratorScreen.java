 ...
 * bar, empty progress arrow) comes from a single 256×256 GUI atlas PNG blitted as the background;
 * only the dynamic layer (energy fill, progress fill) is drawn on top. No readout text is drawn —
...
public class MaceratorScreen extends AbstractContainerScreen<MaceratorMenu> {
	...
	public void extractBackground(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
		...
		// Energy fill: blit the segmented orange sprite (bottom-up), keeping the bar's segment look
		// instead of a flat solid fill. Only the bottom `eFill` rows of the sprite are drawn.
