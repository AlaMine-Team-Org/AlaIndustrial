 ...
 * chunk), so it cannot catch this bug. Instead we drive the whole click through
 * {@code player.gameMode.useItemOn(...)} with the cable actually held in the main hand, which runs
 * {@code BlockState.useItemOn} → {@code useWithoutItem} → (PASS) → {@code ItemStack.useOn} →
 * {@code BlockItem.place} — the exact chain MOD-039 broke.
 *
 * <p><b>MOD-310 — do not shortcut this to {@code someStack.useOn(new UseOnContext(player, hand,
 * hit))} with an empty hand.</b> That context takes its stack from {@code Player#getItemInHand},
 * so it would hold AIR. Vanilla {@code ItemStack#useOn} dispatches on the receiver, so the
 * shortcut still placed the block on Fabric and the test passed for the wrong reason; NeoForge
 * replaces that method with {@code CommonHooks#onPlaceItemIntoWorld}, which dispatches on
...
public final class CablePlacementScenarios {
	 ...
	 * @covers R-PLACE-03
	 */
	public static void mod039_rmbOnCablePlacesAdjacent(GameTestHelper helper) {
		// Place the first cable (setBlock is fine here — it is only the *target* of the click).
		helper.setBlock(CABLE, ModContent.COPPER_CABLE.get());
		helper.assertBlockPresent(ModContent.COPPER_CABLE.get(), CABLE);
		...
		// MOD-310: the cable MUST be in the player's hand, not just passed as the receiver of
		// useOn. `new UseOnContext(player, hand, hit)` fills its stack from
		// Player#getItemInHand(hand) — with an empty hand that is AIR. Vanilla ItemStack#useOn
		// dispatches on `this`, so a bare `cableStack.useOn(ctx)` happened to work on Fabric even
		// with an empty hand; NeoForge replaces that method with CommonHooks#onPlaceItemIntoWorld,
		...
		if (!placed.consumesAction()) {
			helper.fail("RMB with a cable on a cable did not place anything (result " + placed + ")");
		}

		helper.assertBlockPresent(ModContent.COPPER_CABLE.get(), ADJACENT);
		helper.succeed();
	}
	 ...
	 */
	public static void mod038_cableDoesNotConnectToIronChest(GameTestHelper helper) {
		...
		helper.setBlock(chestPos, ModContent.IRON_CHEST.get()); // placed last -> notifies the cable
	 ...
	 */
	public static void cableConnectsOnlyToWindMillBackFace(GameTestHelper helper) {
		...
		for (CableFace cf : rig) {
			helper.setBlock(cf.cablePos(), ModContent.COPPER_CABLE.get());
		}
		helper.setBlock(wind, ModContent.WIND_MILL.get());
	 ...
	 */
	public static void cableConnectsOnlyToWaterMillBackFace(GameTestHelper helper) {
	 ...
	 */
	public static void cableConnectsOnlyToBatteryBoxIOFaces(GameTestHelper helper) {
	...
	public static void mod061_cableDoesNotArmToGeneratorFacing(GameTestHelper helper) {
		assertArmsAround(helper, ModContent.GENERATOR.get(), Direction.NORTH, false);
	}

	/** @implements MOD-061 — geothermal generator front (FACING) face is energy-inert. */
	public static void mod061_cableDoesNotArmToGeothermalFacing(GameTestHelper helper) {
		assertArmsAround(helper, ModContent.GEOTHERMAL_GENERATOR.get(), Direction.NORTH, false);
	}

	/** @implements TC-MACH-001-CON04 — macerator front (FACING) face is energy-inert. */
	public static void mod061_cableDoesNotArmToMaceratorFacing(GameTestHelper helper) {
		assertArmsAround(helper, ModContent.MACERATOR.get(), Direction.NORTH, false);
	}

	/** @implements MOD-061 — electric furnace front (FACING) face is energy-inert. */
	public static void mod061_cableDoesNotArmToElectricFurnaceFacing(GameTestHelper helper) {
		assertArmsAround(helper, ModContent.ELECTRIC_FURNACE.get(), Direction.NORTH, false);
	}

	/** @implements MOD-061 — extractor front (FACING) face is energy-inert. */
	public static void mod061_cableDoesNotArmToExtractorFacing(GameTestHelper helper) {
		assertArmsAround(helper, ModContent.EXTRACTOR.get(), Direction.NORTH, false);
	}

	/** @implements MOD-061 — compressor front (FACING) face is energy-inert. */
	public static void mod061_cableDoesNotArmToCompressorFacing(GameTestHelper helper) {
		assertArmsAround(helper, ModContent.COMPRESSOR.get(), Direction.NORTH, false);
	}
	 ...
	 */
	public static void mod061_cableFacingInertOnNonNorthFacing(GameTestHelper helper) {
		assertArmsAround(helper, ModContent.GENERATOR.get(), Direction.EAST, false);
	}
	 ...
	 */
	public static void mod061_cableDoesNotArmToPumpFacing(GameTestHelper helper) {
		assertArmsAround(helper, ModContent.PUMP.get(), Direction.NORTH, false);
	}
	 ...
	 */
	public static void mod061_cableStaleFlagsReshapeOnFirstTick(GameTestHelper helper) {
	 ...
	 */
	public static void mod071_cableDoesNotArmToSolarPanelUp(GameTestHelper helper) {
		assertArmsAroundPanel(helper, ModContent.SOLAR_PANEL.get());
	}

	/** @implements MOD-071 — Daylight Solar Panel T2, same UP-inert contract as the base panel. */
	public static void mod071_cableDoesNotArmToDaylightSolarPanelUp(GameTestHelper helper) {
		assertArmsAroundPanel(helper, ModContent.DAYLIGHT_SOLAR_PANEL.get());
	}

	/** @implements MOD-071 — Moonlit Solar Panel (night mirror), same UP-inert contract. */
	public static void mod071_cableDoesNotArmToMoonlitSolarPanelUp(GameTestHelper helper) {
		assertArmsAroundPanel(helper, ModContent.MOONLIT_SOLAR_PANEL.get());
	}
}
