 ...
 * {@link RegisterCapabilitiesEvent} at startup. So the registration is wired at startup by
 * {@link NeoForgeGameTestBootstrap} (MOD-242: gametest source set, dev runs only) behind a
 * {@link FMLEnvironment#isProduction()} gate — the same dev-only pattern the {@code /ala demo}
 * stand uses. Nothing here is reachable in a shipped jar (the class is not even packed into it).
 *
