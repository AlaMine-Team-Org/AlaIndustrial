...
public class SolarPanelScreen extends AbstractContainerScreen<SolarPanelMenu> {
    ...
    // ── Energy bar fill zone (x=17..26, y=20..63 = 10×44) ──────────────────────
    // Fill starts at x=17 (covers inner left wall); outer border at x=15 stays dark.
    // Service area source: UV x=176, y=0..43 (orange fill with tick marks at y=7,15,22,30,37).
    private static final int   EBAR_X      = 17;
    private static final int   EBAR_BOTTOM = 64;   // exclusive: fills up to y=63
    ...
    public void extractBackground(GuiGraphicsExtractor graphics, int mouseX, int mouseY,
        ...
        // ── Energy bar fill (bottom-up) — blitted from service area ──────────────
        int capacity = this.menu.getCapacity();
        int energy   = this.menu.getEnergy();
        int eFill    = capacity > 0 ? (int) ((long) energy * EBAR_H / capacity) : 0;
        if (eFill > 0) {
            // Blit the bottom eFill rows of the service fill (preserves tick-mark stripes).
        ...
        if (evoMax > 0 && evoProg > 0) {
            ...
            boolean nightChip = this.menu.getSlot(0).getItem().is(ModItems.ALIGNMENT_CHIP_NIGHT);
