...
public class NetworkGameTest {
	...
	// Ring: GEN(1,2,2) touches two cable arms directly (north + south), each arm runs +x and both
	// rejoin at MAC(4,2,2) (GEN's east neighbour on both z=1 and z=3 rows, closed by two more cables
	// touching MAC from its north and south). Two independent cable paths connect the same two
	// endpoints, closing a cycle in the union-find graph — no diagonal contact anywhere (R-CON-06 safe).
	...
	private void driveRing(GameTestHelper helper, int n) {
		BlockPos[] cables = {RING_A0, RING_A1, RING_A2, RING_A3, RING_B0, RING_B1, RING_B2, RING_B3};
		for (int i = 0; i < n; i++) {
	...
	/**
	 * @implements TC-CABLE-001-CON04 — a ring network (two independent cable paths connecting the same
	 *     generator to the same macerator) delivers EU with no deadlock and no infinite loop: union-find
	 *     merges the two branches into one network the instant they close the cycle at the macerator, and
	 *     {@link NetworkManager#tickAll} still completes in a bounded number of ticks.
	 * @covers R-CON-05
	 *
	 *     <p>TODO(rig): {@code required = false} — the two cable chains (A0..A3, B0..B3) never actually
	 *     touch each other; they only share the macerator as a common non-cable ENDPOINT. Since
	 *     {@link NetworkManager}'s union-find only merges on CABLE-to-cable adjacency (registration in
	 *     {@link NetworkManager#register}), and RING_MAC(4,2,2) sits directly between A3(4,2,1) and
	 *     B3(4,2,3) — 2 blocks apart, not adjacent — there is no cable position where the two chains can
	 *     ever be made to touch without occupying the macerator's own cell. This isn't a tick-timing
	 *     issue (no amount of driving ticks closes a topological gap): the rig needs a real bypass cable
	 *     row connecting the two arms directly (extra width) to form an actual cable ring, which is a
	 *     bigger geometry change than a rig-only fix. Left disabled pending that redesign.
	 */
