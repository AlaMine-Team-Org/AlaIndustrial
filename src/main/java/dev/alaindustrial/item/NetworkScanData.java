 ...
 *
 * <p>{@code producers}/{@code consumers} are endpoint counts; {@code supply}/{@code demand} are the
 * network's instantaneous EU/t estimate at scan time and {@code moved} the EU delivered on the
 * network's most recent tick — the same three figures the actionbar reports.
 */
public record NetworkScanData(int cables, int producers, int consumers, long supply, long demand, long moved) {
