 ...
 * with cached endpoint lists (adjacent producers and consumers, discovered via
 * {@link EnergyStorage#SIDED} on the cables' non-cable neighbours).
 *
...
public final class EnergyNetwork {
	...
	private void refreshEndpoints() {
		...
		for (BlockPos cable : cables) {
			for (Direction dir : Direction.values()) {
				...
				EnergyStorage storage = EnergyStorage.SIDED.find(level, np, dir.getOpposite());
				if (storage == null) {
					continue;
				}
	...
	private EnergyStorage storageAt(Endpoint ep) {
		return EnergyStorage.SIDED.find(level, ep.pos(), ep.side());
	}
	...
	public long tick() {
		...
		for (Endpoint ep : producers) {
			...
			try (Transaction sim = Transaction.openOuter()) {
				supply += st.extract(Long.MAX_VALUE, sim);
				sim.abort();
			}
		}
		...
		for (Endpoint ep : consumers) {
			...
			long r;
			try (Transaction sim = Transaction.openOuter()) {
				r = st.insert(Long.MAX_VALUE, sim);
				sim.abort();
			}
			if (r <= 0) {
				continue;
		...
		try (Transaction tx = Transaction.openOuter()) {
			movedEu += serveClass(machines, liveProducers, remainingSupply, packetCap, tx);
			movedEu += serveClass(sinks, liveProducers, remainingSupply, packetCap, tx);
			tx.commit();
		}
	...
	private long serveClass(List<LiveConsumer> cls, List<LiveProducer> liveProducers,
			long[] remainingSupply, long packetCap, Transaction tx) {
	...
	private record LiveProducer(BlockPos pos, EnergyStorage storage) {
	}

	/** A live consumer endpoint resolved for this tick: its pos, storage and free room. */
	private record LiveConsumer(BlockPos pos, EnergyStorage storage, long room) {
	}
}
